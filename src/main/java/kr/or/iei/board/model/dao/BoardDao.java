...
public class BoardDao {
    ...
    @Autowired
    private BoardRowMapper boardCommentRowMapper;
	...
	public List selectCommentList(int boardNo) {
		String query = "select * from board_comment where board_no=? and board_comment_ref_no is null order by 1";
		List list = jdbc.query(query, boardCommentRowMapper,boardNo);
		return list;
	}

	public List selectReCommentList(int boardNo) {
		String query = "select * from board_comment where board_no=? and board_comment_ref_no is not null order by 1";
		List list = jdbc.query(query, boardCommentRowMapper,boardNo);
		return list;
	}
}
